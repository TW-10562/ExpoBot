  ...
  try {
    ...
    if (fileExtname === '.pdf') {
      ...
      try {
        ...
        console.error('[UploadByPage] Solr indexing failed:', solrErr.message);
        // Don't throw - file is saved to DB, just not searchable
      }

      // Try RAG backend (non-fatal if not running)
      try {
        ...
        console.warn('[UploadByPage] RAG upload failed (non-fatal):', ragErr.message);
      }
      ...
      try {
        ...
        console.warn('[UploadByPage] Solr indexing failed (non-fatal):', solrErr.message);
      }
