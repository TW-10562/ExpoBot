...
const getActiveStatusValue = (dbMode: string) => (dbMode === 'postgres' ? '0' : '1');
...
export const loginVal = async (ctx: Context, next: () => Promise<void>) => {
  ...
  try {
    ...
    if (
      dbMode === 'mysql'
      && !res
      && (userName === 'admin' || userName === 'user' || userName === 'user1' || userName === 'user2')
    ) {
    ...
    let isPasswordValid = await verifyPassword(password, res.password);
    if (!isPasswordValid) {
      // Legacy compatibility for seeded default accounts.
      const legacyDefault =
        ...
        (res.user_name === 'user' && password === '12345');
      if (legacyDefault) {
        await changePasswordSer({ userId: res.user_id, password });
